...
* 调用 `do_mmap` 函数，建立用户 task 的虚拟地址空间信息，包括两个区域:
    * 代码区域：该区域从 ELF 给出的 Segment 起始地址 `phdr->p_offset` 开始，大小为 `uapp_end - uapp_start`(`ramdisk_end - ramdisk_start`)， 权限参考 `phdr->p_flags` 进行设置。
